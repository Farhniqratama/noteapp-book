import{g as d,d as p,S as i}from"./idb-AsxtWpiW.js";class f{constructor(t){this.view=t}async init(){const t=await d(i.favorites);this.view.show(t||[]),this.view.onDelete=async s=>{await p(i.favorites,s);const e=await d(i.favorites);this.view.show(e||[])}}}class y{async render(t){t.innerHTML=`
      <section class="card" style="text-align:center;">
        <h2 style="margin-bottom:1rem;font-size:1.5rem;">Favorit</h2>
        <div id="list" class="favorites-grid"></div>
      </section>
    `,this.presenter=new f(this),await this.presenter.init()}show(t){const s=document.getElementById("list");if(!t||t.length===0){s.innerHTML=`
    ...
    `).join(""),s.querySelectorAll(".del").forEach(e=>{e.onclick=async()=>{var n;const r=e.dataset.id,a=s.querySelector(`.fav-card[data-id="${r}"]`),o=a==null?void 0:a.outerHTML;a.style.transition="opacity .18s ease, transform .18s ease",a.style.opacity="0",a.style.transform="scale(.98)",setTimeout(()=>a==null?void 0:a.remove(),180);try{await((n=this.onDelete)==null?void 0:n.call(this,r))}catch{if(o){const l=document.createElement("div");l.innerHTML=o;const c=l.firstElementChild;s.prepend(c),c.querySelector(".del").onclick=e.onclick}}s.querySelector(".fav-card")||this.show([])}})}}export{y as default};
//# sourceMappingURL=FavoritesView-D8imRw52.js.map
