...
import { getToken, listStories as apiList, addStory as apiAdd } from './storyApi.js'
...
export const StoryModel = {
  ...
  async listAndCache({ page=1, size=10, location=1 } = {}){
    ...
    try{
      const online = await apiList({ page, size, location })
  ...
  // Simpan story: online (API) atau lokal (guest/offline)
  async add({ description, photo, lat, lon }){
    const token = getToken()
    if (token){
      const res = await apiAdd({ description, photo, lat, lon })
      const createdLocal = {
